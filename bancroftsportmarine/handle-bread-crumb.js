jQuery(function ($) {
  ...
  if ($('nav.breadcrumbParent').length) {
    ...
    var modelTitle = '';
    if ($('.model-details-content .model-title').length) {
      $('.model-details-content .model-title').hide();
      modelTitle = $('.model-details-content .model-title').text();
    }
    ...
    new_title =
      modelTitle.length === 0
        ? new_title.replace(/  +/g, ' - ').replace('_', ' ')
        : modelTitle.replace('_', ' ');
