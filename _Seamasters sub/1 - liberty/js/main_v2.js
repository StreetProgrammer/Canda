  ...
  if ($("img[alt='Error:404 Page Not Found']").length) {
    ...
    var content = `<div class="_404-container">
                        <div>
                            ...
                            <div class="row-custom f_hor-c mt-1 f_ver-c f-dir hidden">
...
$(function () {
  if ($('.price_description').eq(0).text().toLowerCase() == 'hide') {
    $('.our-price-div').hide();
    $('.price_description').hide();
  }

  if ($('.no-ads-box').length) {
    $('.row-custom.f_hor-c.mt-1.f_ver-c.f-dir').addClass('hidden');
  }
});

$(document).ajaxComplete(function (event, request, settings) {
  $('.item .price-oem').each(function (elm) {
    if (
      $(this).children('.price-desc').eq(0).text().toLowerCase().trim() ==
      'hide'
    ) {
      $(this).children('.price').hide();
      $(this).children('.price_description').hide();
    }
  });
