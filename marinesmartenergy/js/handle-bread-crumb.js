  ...
  if ($('nav.breadcrumbParent').length) {
    ...
    setTimeout(() => {
      if ($('.custom-breadcrumb').length) {
        var ol = $('.custom-breadcrumb').find('ol.breadcrumb');
        if (ol.children('li').length > 1) {
          ol.children('li:last').remove();
          var content = ol.html();
          $('nav.breadcrumbParent ol.breadcrumb').html(content);

          $('.custom-breadcrumb').hide();
        }
      }
    }, 200);
    /* End */
...
jQuery(function ($) {
  ...
  if ($('#mse-banner').length) {
    $('#mse-banner').addClass('hidden');
  }
});
