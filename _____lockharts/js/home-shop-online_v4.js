var $masterOfFilters = '';
var $filter_url =
  '?filters[lastFilter]=model&modernFilter=true&isStaticFilter=true&filters[page]=1';
var $categories_list = [];
// var $categories_list = {
//   'ATV-UTV': {
...
$(function () {
    ...
    function (data) {
      ...
      $.each($masterOfFilters.model_filter, function (modelkey, modelval) {
          ...
          '<li class="dropdown mega-menu">\
            ...
            '&nbsp;&nbsp;</a>\
													<div class="dropdown-menu mega-level-2">\
														<div class="level-2-hdr">\
															<div>Shop ' +
            modelkey +
            '</div>\
															<div>\
																<a href="#" class="mega-filter-link" data-filter="&filters[model]=' +
            $modelUpdatedKey +
            '">All ' +
            modelkey +
        ...
        $.each(categories, function (catkey, catval) {
          //console.log(catval);
          var $cat_icon =
            $modelClass in $categories_list &&
            catval[1]['text'] in $categories_list[$modelClass]
              ? '<img class="cat-icon" src="https://www.equipmentsearch.com/uploadedimages/8354/' +
                $categories_list[$modelClass][catval[1]['text']] +
                '.jpg">'
              : '';
          $('.' + $modelClass + '-cat-list').append(
            '<li class="level-2-li">' +
              $cat_icon +
              '<a href="#" id="' +
              catval[1]['text'] +
              '" class="mega-filter-link" data-catid="' +
              catval[0] +
              '" data-filter="&filters[categoryId]=' +
              catval[0] +
              '&filters[model]=' +
              $modelUpdatedKey +
              '">' +
          ...
          $.each(sub_categories, function (subkey, subval) {
            $('.' + $modelClass + '-cat-list').append(
              '<li class="level-3-li ' +
                ...
                '"><a href="#" id="' +
                subval[1]['text'] +
                '" class="mega-filter-link" data-filter="&filters[subcategoryId]=' +
                subval[0] +
                '&filters[model]=' +
                $modelUpdatedKey +
                '">' +
          ...
          $('.' + $modelClass + '-cat-list').append(
            '<li class="level-3-li shop-all-link ' +
              catval[0] +
              '"><a href="#" id="' +
              catval[1]['text'] +
              '" class="mega-filter-link" data-filter="&filters[categoryId]=' +
              catval[0] +
              '&filters[model]=' +
              $modelUpdatedKey +
              '">Shop All ' +
      ...
      //if($.browser.device){
      $('.mega-filter-link,.level-2-hdr a').on('click', function (event) {
        event.preventDefault();
        //
        var updated_filter =
          '/shop-online/' + $filter_url + $(this).data('filter');
  ...
  // else{
  // $('.available').after('<div class="shipping-box"><img src="/site-uploads/1001348/Images/shipping_icon_s.jpg" /><span class="shipping-msg">Online orders ship out same day before 12pm. <br/> Orders after 12pm will ship next business day!</span></div>');
  // }

  //}
  if ($supported_pages.includes(pageId)) {
    $('.header-1-nav').append(
      '<a class="shop-online-btn visible-xs" href="#">Shop Online&nbsp;&nbsp;<i class="fas fa-store-alt">&nbsp;</i></a>'
    );
  }
  ...
  $('.shop-online-close-btn, .container.tabs-control-container.open:before').on(
    'click',
    function () {
      event.preventDefault();
...
// $("#comp_1633682142936 .view-details, #comp_1633682142936 .pushstate").each(function(){
// temp = this.href;
// this.href= temp.replace("Home", "Shop-Online");
// });
// });
